...
describe('Evidence surfaces', () => {
  it('exposes exact boundaries, replay download, checks, and immutable artifacts', () => {
    render(<EvidenceRoom mission={testMission()} languageModelName="Gemini 3.5 Flash" />)
    expect(screen.getByRole('heading', { name: 'What the judge receives' })).toBeTruthy()
    ...
    expect(screen.getByText('3 ms · 3.00 MB')).toBeTruthy()
    ...
    expect(screen.getByRole('button', { name: 'Run independent check again' })).toBeTruthy()
    expect(screen.getByRole('button', { name: 'Show the exact Python code' })).toBeTruthy()
    expect(screen.getByText(/not a third-party audit/)).toBeTruthy()
    expect(screen.getByRole('link', { name: /Open source in the repository/ }).getAttribute('href')).toContain('/blob/main/apps/api/constellation/verifier.py')
